...
3. **Eeveelution (Light)** - Pokémon theme
4. **Eeveelution (Dark)** - Pokémon dark mode
...
### Eeveelution Theme
**Inspiration:** Pokémon Eevee evolutions

**Color Palette:**
```
Light Mode:
- Primary: #C4A57B (Eevee brown)
- Secondary: #F5DEB3 (Eevee cream)
...
Dark Mode:
- Primary: #D4A373 (Warm eevee brown)
...
- Eevee mascot in corner (minimally intrusive)
...
**Special Features:**
- Loading screen with Eevee animation
...
**Terminology:**
- Same as Eeveelution theme but with retro styling
...
- **Eeveelution & Pixelated themes**: Animated sprite matches the tagline theme when applicable
...
const spriteMap = {
  ...
  'Eevee': 'eevee-running',
  'default': 'random-eeveelution' // Cycles through all
};
...
│   │   │   │   ├── homestead-light.css
│   │   │   │   ├── homestead-dark.css
│   │   │   │   ├── eeveelution-light.css
│   │   │   │   └── eeveelution-dark.css
│   │   │   └── globals.css
│   │   ├── types/
│   │   ├── utils/
│   │   ├── App.tsx
...
services:
  eeveelution:
    image: eeveelution:latest
    container_name: eeveelution
    environment:
      - PUID=1000
      - PGID=1000
      - TZ=America/Chicago
      - DATABASE_URL=sqlite:///config/eeveelution.db
...
- Compare Prowlarr indexers with Eeveelution indexers
- Auto-add new indexers found in Prowlarr
- Inherit categories, priorities, and capabilities
- Map Prowlarr capabilities to Eeveelution book categories
...
- Auto-add to Eeveelution monitoring
...
- Standardized interface in Eeveelution
...
8. **UI**
   - Theme selection (Homestead/Eeveelution)
   - Dark mode
   - Loading taglines
   - Animation settings
   - Easter egg toggles
...
### Phase 9: Eeveelution Theme (Week 14)
- [ ] Eeveelution theme CSS (light + dark)
...
DATABASE_URL=sqlite:///config/eeveelution.db
# Or for PostgreSQL:
# DATABASE_URL=postgresql://user:pass@localhost/eeveelution
...
This planning document outlines **Eeveelution**, a comprehensive library management system that:

✅ Handles multiple content types (books, audiobooks, comics, magazines, articles)  
✅ Features dual theming (Professional Homestead + Pokémon Eeveelution)  
