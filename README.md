...
- 🔄 **Smart Automation**: Automatic searching, downloading, and library organization
- 🎨 **Dual Themes**: Professional Homestead theme or fun Pokémon-inspired Evolibrary mode
...
- **Quality Profiles**: Prioritize preferred formats (EPUB over MOBI, etc.)
...
- **Evolibrary Theme**: Fun Pokémon-themed interface with evolution-based format icons
...
### Dashboard
![Dashboard](https://via.placeholder.com/800x500/5A5A42/F5EFE6?text=Dashboard+View)

### Library View
![Library](https://via.placeholder.com/800x500/5A5A42/F5EFE6?text=Library+Grid+View)

### Book Details
![Book Details](https://via.placeholder.com/800x500/5A5A42/F5EFE6?text=Book+Details)

### Evolibrary Theme
![Evolibrary Theme](https://via.placeholder.com/800x500/C4A57B/FFF9F0?text=Evolibrary+Theme)

### Mobile View
![Mobile](https://via.placeholder.com/400x700/5A5A42/F5EFE6?text=Mobile+Responsive)
...
- [Quality Profiles](docs/QUALITY_PROFILES.md)
...
`self-hosted` `docker` `library-management` `ebook-manager` `audiobook-manager` `comic-manager` `book-automation` `radarr` `sonarr` `prowlarr` `jackett` `kavita` `calibre-alternative` `readarr-alternative` `lazylibrarian-alternative` `arr-stack` `media-server` `homelab` `plex-alternative` `jellyfin` `python` `fastapi` `react` `typescript` `open-source` `free-software` `book-downloader` `ebook-downloader` `audiobook-downloader` `comic-downloader` `magazine-manager` `reading-tracker` `book-organizer` `metadata-manager` `book-collection` `digital-library` `personal-library` `book-catalog` `series-tracker` `author-tracker` `release-calendar` `quality-profiles` `download-automation` `usenet` `torrent` `deluge` `qbittorrent` `transmission` `sabnzbd` `goodreads` `google-books` `openlibrary` `epub` `mobi` `pdf` `cbz` `m4b` `linux` `windows` `macos` `raspberry-pi` `unraid` `synology` `truenas` `pokemon-theme` `evolibrary` `bibliophile` `book-lover` `reading` `books` `audiobooks` `comics` `magazines`
...
- book quality profiles like radarr
